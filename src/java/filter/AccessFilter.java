...
public class AccessFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        String domainName = "http://" + serverName + ":" + port + "/onlinelearn";
        if (user != null) {
            String actualPath = String.valueOf(req.getRequestURL());
            actualPath = actualPath.replace(domainName, "");
            System.out.println(actualPath);
            FeatureDBContext fdb = new FeatureDBContext();
            if (!fdb.checkContextPath(actualPath, user.getUsername())) {
                resp.sendRedirect(domainName + "/view/static/404.html");
            } else {
            ...
            if ((contextPath.contains("authen"))) {
                domainName += "/login";
                resp.sendRedirect(domainName);
            }
        }
    }
